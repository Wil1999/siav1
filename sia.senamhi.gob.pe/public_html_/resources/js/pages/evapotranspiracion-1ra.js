...
function createMap() {
    ...
    sourceReser.getLayer('04_06_001_03_001_531_0000_00_00').addTo(mymap);
...
async function getLugaresAfectados() {
    ...
    $('.depBuscarZoom').on('click', function () { 
        ...
        console.log(product_esquema)
        $('.depBuscarZoom').removeClass('active');
        $(this).addClass('active');
        
        var styleColor = {
            color: 'red',/* color de contorno del mapa*/
            weight: 1.5,
            fill: false,
            fillOpacity: 0.15
        };
        ...
        console.log(`http://sia.senamhi.gob.pe:8080/sia/dashboard/monitoreo/${product_esquema}/${product_tabla}/${product_anio}/${depName.replaceAll('-', ' ').toUpperCase()}`);
    ...
    $('.nav .nav-item').on('click', function (e) {
        ...
        console.log(leyendaName)
        console.log($('.dep_' + leyendaName).length)
...
function getHistorialProduct() {
    ...
    console.log(`http://sia.senamhi.gob.pe:8080/sia/producto/anio/${product_esquema}/${product_tabla}`)
...
function getDetailsProduct() {
  ...
  //  $('.body .header .title .tooltip .eva_mes').html(` ${product_grupoNomCorto} `);
  if (product_grupoNomCorto == null || product_grupoNomCorto=="") { product_grupoNomCorto = "Evapotranspiración Potencial (AGO)"; } 
