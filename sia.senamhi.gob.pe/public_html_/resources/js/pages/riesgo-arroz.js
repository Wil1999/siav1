...
function createMap() {
    ...
    function load_monitoreos(filter) {
        ...
        sourceReser = new My_Source('https://idesep.senamhi.gob.pe/geoserver/g_11_01/wms?', {
            cql_filter: filter,
    ...
    sourceReser.getLayer('11_01_001_03_001_531_0000_00_00').addTo(mymap);
...
async function getLugaresAfectados() {
    ...
    $('.depTotales').on('click', function () {
        ...
        /*
        $.get(`http://sia.senamhi.gob.pe:8080/sia/dashboard/riesgo/${product_esquema}/${product_tabla}/${product_anio}/${nombreDepProv[0].replaceAll('-', ' ').toUpperCase()}/${nombreDepProv[1].replaceAll('-', ' ').toUpperCase()}`)
            .done(function (data) {
                if (data) {
                    let series = data.series;
...
function getDetailsProduct() {
    ...
    $('.toggle2 .titulo1').html(`Pronóstico de Riesgo Agroclimático - ${product_grupoNomCorto}`);
